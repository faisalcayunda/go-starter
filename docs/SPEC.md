...
| **Add-on** | Komponen opsional multi-select non-arsitektural: Docker+compose, Makefile/Taskfile, CI, golangci-lint, observability (otel+health), auth scaffold (JWT), `.env.example` (§4.8 `q_addons`), `strapgorm` (endpoint list gaya Strapi `GET /api/products` di atas GORM via `github.com/faisalcayunda/strapgorm`; bounded — lihat C-strapgorm). |
...
| C-strapgorm | Add-on `strapgorm` | requires | add-on `strapgorm` aktif (`--addons`/`--feature strapgorm`) ∧ ¬(`--access = gorm` ∧ `--db ∈ {postgres,mysql}`) | **Invalid.** Add-on `strapgorm` (query builder gaya Strapi di atas GORM) butuh akses query GORM + driver SQL `postgres`/`mysql`. Didukung di KETIGA arsitektur: **monolith** REUSE `*gorm.DB` access=gorm (tanpa pool kedua); **modular-monolith** domain modular `internal/modules/product/**` di-inject `*gorm.DB` access=gorm lewat composition root (anchor `region:modules`); **microservice** service `product` mandiri (gRPC Ping + HTTP `/api/products`) dgn koneksi GORM per-service (modul `feature-strapgorm-microservice` + driver `-postgres`/`-mysql`; go.mod jujur — hanya driver terpilih). Prasyarat ditegakkan `answers.Validate` (otoritas field-level, pesan ramah tunggal) lalu dicermin resolver `checkConstraints` sebagai lapis kedua fail-fast. Aktif → go directive project dinaikkan ke 1.25 (strapgorm butuh Go ≥ 1.25) + dep `github.com/faisalcayunda/strapgorm` dipin `v0.0.0-20260610233751-7c87a8f27fb1`. |
