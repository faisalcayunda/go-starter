...
// db DIPINJAM dari koneksi access=gorm (REUSE, lihat wiring.go) — adapter ini
// TIDAK membuka koneksi sendiri.
type gormRepository struct {
	db *gorm.DB
}

// NewGormRepository membuat Repository di atas koneksi GORM yang ada. Pemanggil
// (wiring.go) meneruskan *gorm.DB yang SAMA dengan access=gorm.
